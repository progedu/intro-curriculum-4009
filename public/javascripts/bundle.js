...
/***/ function(module, exports, __webpack_require__) {
	...
	const loadavg = $('#loadavg'); // index.jade になるpタグのid（この場所にappendChild的なことをしている）

	const socket = __webpack_require__(2)('http://localhost:8000');
	socket.on('server-status', (data) => {
	  loadavg.text(data.loadavg.toString() + ', ' + data.loadavg + ', ' + data.uptime); // data.loadavg は配列なので、わかりやすく
	});
